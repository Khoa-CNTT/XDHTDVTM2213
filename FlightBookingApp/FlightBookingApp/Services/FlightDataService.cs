...
namespace FlightBookingApp.Services
{
    public class FlightDataService
    {
        ...
        private async Task<int> SaveTimetableToDatabaseAsync(JsonElement data, string departureAirportCode, Dictionary<string, int> airportIds)
        {
            ...
            // Lấy danh sách tất cả các hãng hàng không từ bảng Airlines để so sánh
            var airlines = await _context.Airlines.ToDictionaryAsync(a => a.Name, a => a.AirlineId, StringComparer.OrdinalIgnoreCase);
            ...
            foreach (var flight in data.EnumerateArray())
            {
                try
                {
                    ...
                    string airlineName = GetJsonProperty(flight, "airline", "name")?.GetString() ?? "Unknown Airline";
                    string status = GetJsonProperty(flight, "status")?.GetString() ?? "Scheduled";

                    // Tìm AirlineId từ bảng Airlines dựa trên tên hãng hàng không
                    int? airlineId = null;
                    if (airlines.TryGetValue(airlineName, out var foundAirlineId))
                    {
                        airlineId = foundAirlineId;
                    }
                    else
                    {
                        // Nếu không tìm thấy hãng hàng không, có thể để AirlineId là null
                        // Hoặc thêm logic để tạo mới một bản ghi trong bảng Airlines nếu cần
                        Console.WriteLine($"Airline '{airlineName}' not found in Airlines table. Setting AirlineId to null.");
                    }
                    ...
                    decimal price = CalculatePriceBasedOnDistanceAndAirline(distanceKm, airlineName);
                    ...
                    var newFlight = new Flight
                    {
                        ...
                        Airline = airlineName, // Lưu tên hãng hàng không
                        AirlineId = airlineId, // Lưu AirlineId
                        Price = price,
                        AvailableSeats = 100,
                        Stops = stops,
                        Status = status
