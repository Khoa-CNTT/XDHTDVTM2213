...
// Register NEW services for testing
builder.Services.AddScoped<NewFlightDataService>();
builder.Services.AddScoped<NewFlightSyncService>();
...
{
    options.SchedulePollingInterval = TimeSpan.FromMinutes(1); // Kiểm tra lịch trình mỗi phút
});
...
// Lập lịch đồng bộ dữ liệu lúc 11h tối mỗi ngày (NewFlightSyncService)
RecurringJob.AddOrUpdate<NewFlightSyncService>(
    "new-flight-sync-job",
    service => service.SyncFlightsAsync(),
    "0 23 * * *", // Cron: Chạy lúc 23:00 mỗi ngày
    TimeZoneInfo.Local);

// Lập lịch đồng bộ dữ liệu lúc 11h15 tối mỗi ngày (FutureFlightSyncService - Aviation Edge API)
RecurringJob.AddOrUpdate<FutureFlightSyncService>(
    "future-flight-sync-job",
    service => service.SyncFlightsAsync(),
    "15 23 * * *", // Cron: Chạy lúc 23:15 mỗi ngày
    TimeZoneInfo.Local);

// Lập lịch xóa dữ liệu cũ lúc 11h30 tối mỗi ngày (DataCleanupService)
